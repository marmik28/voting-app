...
import { VOTINGAPP_PROGRAM_ADDRESS } from '../programs';
...
export type DecrementInstruction<
  TProgram extends string = typeof VOTINGAPP_PROGRAM_ADDRESS,
...
export type DecrementInput<TAccountVotingapp extends string = string> = {
  votingapp: Address<TAccountVotingapp>;
};

export function getDecrementInstruction<
  TAccountVotingapp extends string,
  TProgramAddress extends Address = typeof VOTINGAPP_PROGRAM_ADDRESS,
>(
  input: DecrementInput<TAccountVotingapp>,
  config?: { programAddress?: TProgramAddress }
): DecrementInstruction<TProgramAddress, TAccountVotingapp> {
  // Program address.
  const programAddress = config?.programAddress ?? VOTINGAPP_PROGRAM_ADDRESS;

  // Original accounts.
  const originalAccounts = {
    votingapp: { value: input.votingapp ?? null, isWritable: true },
  };
  ...
  const instruction = {
    accounts: [getAccountMeta(accounts.votingapp)],
    programAddress,
    data: getDecrementInstructionDataEncoder().encode({}),
  } as DecrementInstruction<TProgramAddress, TAccountVotingapp>;
